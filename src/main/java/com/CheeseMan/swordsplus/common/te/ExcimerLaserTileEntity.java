...
public class ExcimerLaserTileEntity extends LockableLootTileEntity implements ITickableTileEntity, INamedContainerProvider {
    ...
    public static int slots = 4, WORKING_TIME = 60, RF_PER_TICK = 1;
    ...
    public void tick() {
        ...
        if (recipe == null || !canStartWorking(recipe) || !tryReduceEnergyFromBattery()) {
            reset();
            return;
        }
        if (counter <= 0) {
    ...
    /**
     * reduces the energy if possible
     */
    protected boolean tryReduceEnergyFromBattery(){
        ItemStack battery = getItem(0);
        //get the energy from the stack
        IEnergyStorage storage = battery.getCapability(CapabilityEnergy.ENERGY).orElse(null);
        if(storage != null){
            //checks if it can extract the energy per Tick
            int extracted = storage.extractEnergy(RF_PER_TICK, true);
            if (extracted == RF_PER_TICK){
                //extracts it then
                storage.extractEnergy(RF_PER_TICK, false);
                return true;
            }
        }
        return false;
